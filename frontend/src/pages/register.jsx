    ...
    <Layout>
      <div className="min-h-screen flex flex-col items-center justify-center py-10 px-4">
        ...
        <div className="bg-white border border-slate-200 p-8 rounded-xl shadow-xl w-full max-w-md">
          <h1 className="text-2xl font-bold text-slate-900 mb-1 font-sans">Sign up</h1>
          <p className="text-slate-500 text-xs mb-6 font-sans">Make the most of your professional life</p>

          {isError && (
            <div className="bg-red-50 text-red-600 p-3 rounded-lg text-xs font-semibold mb-4 border border-red-200 shadow-sm font-sans">
              {message?.message || message || "Registration failed"}
            </div>
          )}

          {isSuccess && (
            <div className="bg-green-50 text-green-700 p-3 rounded-lg text-xs font-semibold mb-4 border border-green-200 shadow-sm font-sans">
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            <div>
              ...
              <input
                ...
                className="w-full border border-slate-300 rounded px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-[#0077b5] focus:border-[#0077b5] text-slate-800 placeholder-slate-400"
          ...
          <div className="mt-8 text-center text-xs text-slate-500 font-sans">
            Already on LinkedIn?{" "}
            <Link href="/login" className="text-[#0077b5] font-semibold hover:underline">
              Sign in
            </Link>
          </div>
        </div>
